...
export default function BlogPost({ post, isDeleting, setIsDeleting, user }: { post: Post, isDeleting: boolean, setIsDeleting: React.Dispatch<React.SetStateAction<boolean>>, user: User }) {
  ...
  // return (
  //   <div>
  //     You are not currently logged in. Please login to continue.
  //   </div>
  // )

  return (
    <div>
      <div className={`bg-white px-6 py-16 lg:py-32 lg:px-8 ${isModalOpen ? "brightness-50" : ""}`}>
      ...
      </div>
      {
        isModalOpen ? (
          <div className="bg-white fixed top-[40%] mx-12 lg:left-[30%] md:left-[20%] sm:left-[10%] 2xl:top-[40%] 2xl:left-[40%] p-8 shadow-lg border rounded-lg">
            <span className="">You are not currently logged in. Please login to continue.</span>
            <div className="flex justify-end space-x-4 mt-8">
              <button className="text-sm font-medium" onClick={() => setIsModalOpen(!isModalOpen)}>Cancel</button>
              <Link href="/login" className="rounded border py-2 px-4 bg-black text-white text-sm font-medium">Login</Link>
            </div>
            {/* <p></p>
